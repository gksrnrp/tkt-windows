...
import { withImeCursor } from './ime-cursor.js'
...
async function setup(): Promise<Config> {
  ...
  try {
    ...
    // Windows 는 카카오톡이 복사해준 텍스트만 볼 수 있어서 말풍선 위치를 알 수 없다.
    // 내 이름을 알아야 내가 보낸 메시지를 가려낼 수 있다.
    if (process.platform === 'win32') {
      console.log('\n카카오톡에 표시되는 본인 이름을 알려주세요.')
      console.log('내가 보낸 메시지를 구분하는 데만 씁니다. 비워두면 전부 상대 메시지로 보입니다.')
      myName = (await rl.question('본인 이름: ')).trim()
    }
  } finally {
    rl.close()
  }
...
설정 파일: ${configPath}`)
  ...
  // 한글 조합 중인 글자가 입력줄 아래로 떨어지지 않도록 커서를 손봐야 한다. 자세한 사정은
  // ime-cursor.ts 에 적어뒀다.
  const ime = withImeCursor(process.stdout)
  try {
    const app = render(<App config={config} />, { stdout: ime.stdout })
